		...
		<el-form ref="form" :model="form" status-icon label-width="120px" v-if="is_view()">
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','employee_number') || $check_field('add','employee_number') || $check_field('set','employee_number')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="员工编号" prop="employee_number">
					<el-input id="employee_number" v-model="form_sub['employee_number']" placeholder="请输入员工编号"
							  v-if="user_group === '管理员' || (form_sub['employee_user_id'] && $check_field('set','employee_number')) || (!form_sub['employee_user_id'] && $check_field('add','employee_number'))"></el-input>
					<div v-else-if="$check_field('get','employee_number')">{{form_sub['employee_number']}}</div>
				</el-form-item>
			</el-col>
			<el-col v-if="user_group === '管理员' || $check_field('get','full_name') || $check_field('add','full_name') || $check_field('set','full_name')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="姓名" prop="full_name">
					<el-input id="full_name" v-model="form_sub['full_name']" placeholder="请输入姓名"
							  v-if="user_group === '管理员' || (form_sub['employee_user_id'] && $check_field('set','full_name')) || (!form_sub['employee_user_id'] && $check_field('add','full_name'))"></el-input>
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','gender') || $check_field('add','gender') || $check_field('set','gender')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="性别" prop="gender">
					<el-select id="gender" v-model="form_sub['gender']"
						v-if="user_group === '管理员' || (form_sub['employee_user_id'] && $check_field('set','gender')) || (!form_sub['employee_user_id'] && $check_field('add','gender'))">
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','salary') || $check_field('add','salary') || $check_field('set','salary')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="薪资" prop="salary">
					<el-input id="salary" v-model="form_sub['salary']" placeholder="请输入薪资"
							  v-if="user_group === '管理员' || (form_sub['employee_user_id'] && $check_field('set','salary')) || (!form_sub['employee_user_id'] && $check_field('add','salary'))"></el-input>
					<div v-else-if="$check_field('get','salary')">{{form_sub['salary']}}</div>
				</el-form-item>
			</el-col>
			<el-col v-if="user_group === '管理员' || $check_field('get','department') || $check_field('add','department') || $check_field('set','department')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="部门" prop="department">
					<el-input id="department" v-model="form_sub['department']" placeholder="请输入部门"
							  v-if="user_group === '管理员' || (form_sub['employee_user_id'] && $check_field('set','department')) || (!form_sub['employee_user_id'] && $check_field('add','department'))"></el-input>
					<div v-else-if="$check_field('get','department')">{{form_sub['department']}}</div>
				</el-form-item>
			</el-col>
			<el-col v-if="user_group === '管理员' || $check_field('get','post') || $check_field('add','post') || $check_field('set','post')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="职务" prop="post">
					<el-input id="post" v-model="form_sub['post']" placeholder="请输入职务"
							  v-if="user_group === '管理员' || (form_sub['employee_user_id'] && $check_field('set','post')) || (!form_sub['employee_user_id'] && $check_field('add','post'))"></el-input>
...
<script>
	...
	export default {
        ...
        form_sub = this.events("submit_before", Object.assign({}, form_sub)) || form_sub;
				if(form_sub["employee_user_id"]){
					// 提交事件
					this.$post("~/api/" + this.group_table + "/set?employee_user_id=" + form_sub["employee_user_id"],form_sub,(res)=>{
			...
			is_view(){
				...
				if(!bl){
					bl = this.$check_action('/employee_user/table','add');
					console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
				}
				if(!bl){
					bl = this.$check_action('/employee_user/table','set');
					console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
				}
				if(!bl){
					bl = this.$check_action('/employee_user/view','add');
					console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
				}
				if(!bl){
					bl = this.$check_action('/employee_user/view','set');
					console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
				}
				if(!bl){
					bl = this.$check_action('/employee_user/view','get');
