			...
			<el-col v-if="user_group === '管理员' || $check_field('get','staff') || $check_field('add','staff') || $check_field('set','staff')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="员工" prop="staff">
						<div v-if="user_group !== '管理员'">
							...
							<!--v-if="user_group === '管理员' || (form['attendance_information_id'] && $check_field('set','staff')) || (!form['attendance_information_id'] && $check_field('add','staff'))" :disabled="disabledObj['staff_isDisabled']"></el-input>-->
							<!--<div v-else-if="$check_field('get','staff')">{{form['staff']}}</div>-->
							<el-select v-if="user_group === '管理员' || (form['attendance_information_id'] && $check_field('set','staff')) || (!form['attendance_information_id'] && $check_field('add','staff'))" id="staff" v-model="form['staff']" :disabled="disabledObj['staff_isDisabled']">
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','employee_number') || $check_field('add','employee_number') || $check_field('set','employee_number')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="员工编号" prop="employee_number">
					<el-input id="employee_number" v-model="form['employee_number']" placeholder="请输入员工编号"
							  v-if="user_group === '管理员' || (form['attendance_information_id'] && $check_field('set','employee_number')) || (!form['attendance_information_id'] && $check_field('add','employee_number'))" :disabled="disabledObj['employee_number_isDisabled']"></el-input>
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','full_name') || $check_field('add','full_name') || $check_field('set','full_name')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="姓名" prop="full_name">
					<el-input id="full_name" v-model="form['full_name']" placeholder="请输入姓名"
							  v-if="user_group === '管理员' || (form['attendance_information_id'] && $check_field('set','full_name')) || (!form['attendance_information_id'] && $check_field('add','full_name'))" :disabled="disabledObj['full_name_isDisabled']"></el-input>
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','department') || $check_field('add','department') || $check_field('set','department')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="部门" prop="department">
					<el-input id="department" v-model="form['department']" placeholder="请输入部门"
							  v-if="user_group === '管理员' || (form['attendance_information_id'] && $check_field('set','department')) || (!form['attendance_information_id'] && $check_field('add','department'))" :disabled="disabledObj['department_isDisabled']"></el-input>
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','post') || $check_field('add','post') || $check_field('set','post')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="职务" prop="post">
					<el-input id="post" v-model="form['post']" placeholder="请输入职务"
							  v-if="user_group === '管理员' || (form['attendance_information_id'] && $check_field('set','post')) || (!form['attendance_information_id'] && $check_field('add','post'))" :disabled="disabledObj['post_isDisabled']"></el-input>
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','attendance_status') || $check_field('add','attendance_status') || $check_field('set','attendance_status')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="考勤状态" prop="attendance_status">
					<el-select id="attendance_status" v-model="form['attendance_status']"
						v-if="user_group === '管理员' || (form['attendance_information_id'] && $check_field('set','attendance_status')) || (!form['attendance_information_id'] && $check_field('add','attendance_status'))">
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','remarks') || $check_field('add','remarks') || $check_field('set','remarks')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="备注" prop="remarks">
					<el-input id="remarks" v-model="form['remarks']" placeholder="请输入备注"
							  v-if="user_group === '管理员' || (form['attendance_information_id'] && $check_field('set','remarks')) || (!form['attendance_information_id'] && $check_field('add','remarks'))" :disabled="disabledObj['remarks_isDisabled']"></el-input>
...
<script>
	...
	export default {
		...
		data() {
			return {
				field: "attendance_information_id",
				url_add: "~/api/attendance_information/add?",
				url_set: "~/api/attendance_information/set?",
				url_get_obj: "~/api/attendance_information/get_obj?",
				url_upload: "~/api/attendance_information/upload?",

				query: {
					"attendance_information_id": 0,
				},
				...
				form: {
					...
					"remarks":'', // 备注
					"attendance_information_id": 0, // ID
				...
				group_user_staff: "",
				//考勤状态选项列表
				list_attendance_status: ['正常','迟到','早退','旷工','休假'],
		...
		methods: {
			...
			is_view(){
				...
				if(!bl){
					bl = this.$check_action('/attendance_information/table','add');
					console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
				}
				if(!bl){
					bl = this.$check_action('/attendance_information/table','set');
					console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
				}
				if(!bl){
					bl = this.$check_action('/attendance_information/view','add');
					console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
				}
				if(!bl){
					bl = this.$check_action('/attendance_information/view','set');
					console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
				}
				if(!bl){
					bl = this.$check_action('/attendance_information/view','get');
