<template>
	<div class="diy_details_box diy_div_department_information">
		<div class="warp">
			<div class="container">
	...
	export default {
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					source_table: "department_information",
					source_field: "department_information_id",
					source_id: this.obj["department_information_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/department_information/set?department_information_id=' + _this.obj["department_information_id"], {
								praise_len
							}, (res) => {
								if(res.result){
