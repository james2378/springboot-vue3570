<template>
	<div class="diy_details_box diy_div_salary_information">
		<div class="warp">
			<div class="container">
	...
	export default {
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					source_table: "salary_information",
					source_field: "salary_information_id",
					source_id: this.obj["salary_information_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/salary_information/set?salary_information_id=' + _this.obj["salary_information_id"], {
								praise_len
							}, (res) => {
								if(res.result){
