<template>
	<div class="diy_details_box diy_div_employee_user">
		<div class="warp">
			<div class="container">
				...
				<div class="row">
						...
						<!-- 图片 -->
							...
							<div class="details_btn_wrap">
								<button class="btn btn-primary details_btn" v-if="$check_action('/employee_user/details','set')" @click="change_collect()" >收藏</button>
							</div>
						<!-- 富文本 -->
	...
	export default {
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					source_table: "employee_user",
					source_field: "employee_user_id",
					source_id: this.obj["employee_user_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/employee_user/set?employee_user_id=' + _this.obj["employee_user_id"], {
								praise_len
							}, (res) => {
								if(res.result){
