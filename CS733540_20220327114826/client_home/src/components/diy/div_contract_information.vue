<template>
	<div class="diy_details_box diy_div_contract_information">
		<div class="warp">
			<div class="container">
	...
	export default {
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					source_table: "contract_information",
					source_field: "contract_information_id",
					source_id: this.obj["contract_information_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/contract_information/set?contract_information_id=' + _this.obj["contract_information_id"], {
								praise_len
							}, (res) => {
								if(res.result){
